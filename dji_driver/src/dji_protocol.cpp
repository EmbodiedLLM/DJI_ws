...
const uint16_t CRC16_TABLE[256] =
{
    ...
    0x9cc9, 0x8d40, 0xbddb, 0xac52, 0xd8ed, 0xc964, 0xfbfb, 0xea72,
    0x2306, 0x328f, 0x0014, 0x119d, 0x6522, 0x74ab, 0x4630, 0x57b9,
    0xad4e, 0xbcc7, 0x8e5c, 0x9fd5, 0xeb6a, 0xfae3, 0xc878, 0xd9f1,
    0x3187, 0x200e, 0x1295, 0x031c, 0x77a3, 0x662a, 0x54b1, 0x4538,
...
{
    // 计算并填充CRC8（帧头校验，前4字节）
    cmd.crc8 = calculate_crc8(reinterpret_cast<uint8_t*>(&cmd), 4, CRC8_INIT);
    
    // 计算并填充CRC16（整个数据包校验，不包括CRC16本身）
    cmd.crc16 = calculate_crc16(reinterpret_cast<uint8_t*>(&cmd), 
